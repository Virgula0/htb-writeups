...
class AssociationsModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		if (!empty($fields['catid']))
		{
			...
			$query->select($db->qn('c.title', 'category_title'))
				->join('LEFT', $db->qn('#__jdownloads_categories', 'c') . ' ON ' . $db->qn('c.id') . ' = ' . $db->qn($fields['catid']));
	...
	public function purge($context = '', $key = '')
	{
		...
		// Delete only all jD associations
        $query->where($db->qn('context') . ' LIKE ' . $db->quote('com_jdownloads.%'));
		...
		{
			$app->enqueueMessage(Text::_('COM_JDOWNLOADS_ASSOCIATIONS_PURGE_FAILED'), 'error');

			return false;
		}

		$app->enqueueMessage(
			Text::_((int) $db->getAffectedRows() > 0 ? 'COM_JDOWNLOADS_ASSOCIATIONS_PURGE_SUCCESS' : 'COM_JDOWNLOADS_ASSOCIATIONS_PURGE_NONE'),
			'message'
		);
	...
	public function clean($context = '', $key = '')
	{
		...
		foreach ($assocKeys as $value)
		{
			...
			{
				$app->enqueueMessage(Text::_('COM_JDOWNLOADS_ASSOCIATIONS_DELETE_ORPHANS_FAILED'), 'error');
		...
		$app->enqueueMessage(
			Text::_($count > 0 ? 'COM_JDOWNLOADS_ASSOCIATIONS_DELETE_ORPHANS_SUCCESS' : 'COM_JDOWNLOADS_ASSOCIATIONS_DELETE_ORPHANS_NONE'),
			'message'
		);
