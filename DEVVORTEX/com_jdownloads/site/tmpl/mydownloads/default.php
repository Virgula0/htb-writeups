...
use JDownloads\Component\JDownloads\Site\Helper\RouteHelper;
    ...
    if ($subheader != ''){
        ...
        $subheader = str_replace('{subheader_title}', Text::_('COM_JDOWNLOADS_MY_DOWNLOADS'), $subheader);
    ...
    if ($layout_files_text != ''){
        ...
        for ($i = 0; $i < count($items); $i++) {
            ...
            if ($items[$i]->featured){
                ...
                if ($params->get('featured_pic_filename')){
                    ...
                    $featured_pic = '<img class="jd_featured_star" src="'.Uri::base().'images/jdownloads/featuredimages/'.$params->get('featured_pic_filename').'" width="'.$params->get('featured_pic_size').'" height="'.$params->get('featured_pic_size_height').'" alt="'.substr($featured_pic_name,0,-4).'" />';
            ...
            if ($items[$i]->params->get('access-download') == true){ 
                ...
                $link_button = $download_link;
				
                // Need to setup for license etc check separately to force summary view
                if ($items[$i]->license || $items[$i]->password || $jd_user_settings->view_captcha) {
                ...
                if ($url_task == 'download.send'){ 
                    $download_link_text = '<a '.$blank_window.' href="'.$download_link.'" title="'.Text::_('COM_JDOWNLOADS_LINKTEXT_DOWNLOAD_URL').'" class="jdbutton '.$download_color.' '.$download_size_listings.'">';
                } else {
                    $download_link_text = '<a href="'.$download_link.'" title="'.Text::_('COM_JDOWNLOADS_LINKTEXT_DOWNLOAD_URL').'" class="jdbutton '.$download_color.' '.$download_size_listings.'">';
                }    
             
                // When Download has no file or Download is not enabled then do not show Download button
                if ($has_no_file || !$items[$i]->state){
                    ...
                    if ($layout_has_download){
                        ...
                        $html_file = str_replace('{checkbox_list}', $download_link_text.Text::_('COM_JDOWNLOADS_LINKTEXT_DOWNLOAD_URL').'</a>', $html_file);  
                    }
                }    
            ...
            if ($params->get('view_detailsite')){
                $title_link_text = '<a '.$blank_window.' href="'.$title_link.'">'.$this->escape($items[$i]->title).'</a>';
			}  else {
        ...
        if (isset($user_can_see_download_url)){ 
            $checkbox_top = '<tr><form name="down'.$formid.'" action="'.Route::_('index.php?option=com_jdownloads&amp;view=summary&amp;Itemid='.$file_itemid).'"
                    onsubmit="return pruefen('.$formid.',\''.Text::_('COM_JDOWNLOADS_JAVASCRIPT_TEXT_1').' '.Text::_('COM_JDOWNLOADS_JAVASCRIPT_TEXT_2').'\');" method="post">
            ...
            if ($user->guest){
                $regg = str_replace('<br />', '', Text::_('COM_JDOWNLOADS_FRONTEND_FILE_ACCESS_REGGED'));
            } else {
        ...
        $body .= HTMLHelper::_( 'form.token' ).'</form>';
