 ...
 *
 */
class SummaryModel extends DownloadModel
{
	/**
	...
	public function getItems($pk = null)
	{
		...
		$pk              = (int)$this->getState('download.id', 0);
        $marked_files_id = $this->getState('download.marked_files.id', 0);
        
        if ($marked_files_id === 0){
            $marked_files_id = array();
        }
		...
		if (!isset($this->_items[$pk])) {
			...
			try {
				...
				$query->select($this->getState(
                    ...
                    'a.changelog, a.password_md5, a.views, a.update_active, a.published, a.user_access, ' .
                ...
                foreach ($files as $file)
                {
                    ...
                    if (!$file->url_download && $file->other_file_id > 0){
                        // We need the file name from the source Download item in this case (to view it in Summary and Log) 
                        $db = Factory::getDbo();
                        $query = $db->getQuery(true);
                        $query->select('url_download, size');
                        $query->from($db->quoteName('#__jdownloads_files'));
                        $query->where($db->quoteName('id')." = ".$db->quote($file->other_file_id));
                        $db->setQuery($query);

                        $result = $db->loadObject();

                        if ($result){
                            $file->other_file_name = $result->url_download;
                            $file->other_file_size = $result->size;
                        } else {
                            $file->other_file_name = '';                            
                            $file->other_file_size = '';
                        }
                    }
            ...
            // We have no valid downloads to view. Possibly because a user has logged in or logged out of the Summary page.
            $app = Factory::getApplication();
            // We redirect to the start page
    ...
    public function getForm($data = array(), $loadData = true) 
    {
        ...
        // Get the form.
        $form = $this->loadForm('com_jdownloads.summary', 'summary', array('control' => 'jform', 'load_data' => $loadData));
