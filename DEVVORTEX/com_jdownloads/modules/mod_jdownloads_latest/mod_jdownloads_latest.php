...
use JDownloads\Module\JDownloadsLatest\Site\Helper\JDownloadsLatestHelper;
...
use Joomla\CMS\Language\Text;
    ...
    $text_before           = JDownloadsLatestHelper::getOnlyLanguageSubstring($before);
	}
    $text_after           = '';  // assume no text after
	$after = $params->get( 'text_after' );
	if($after != '') {
		$after        = trim($after);	//only here if not empty
    $text_after            = JDownloadsLatestHelper::getOnlyLanguageSubstring($after);
	}
    ...
    $view_date_text        = JdownloadsLatestHelper::getOnlyLanguageSubstring($view_date_text);	
    ...
    $cat_show_text         = JDownloadsLatestHelper::getOnlyLanguageSubstring($cat_show_text);
    ...
    $files = JDownloadsLatestHelper::getList($params, $model);
	...
	require ModuleHelper::getLayoutPath('mod_jdownloads_latest', $layout);
?>